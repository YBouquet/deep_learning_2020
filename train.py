...
def pretrain_train_model(model, train_input, train_target, train_figures_target, k_fold, mini_batch_size, lr, num_epoch, auxiliary_loss=True, decrease_lr = False):
    criterion =nn.BCEWithLogitsLoss()
    auxiliary_criterion = nn.CrossEntropyLoss()
    optimizer = optim.SGD(model.parameters(), lr=lr)
    ...
    for k in range(k_fold):
        ...
        epochs = {PRETRAINING : 50, TRAINING : num_epoch}
        lrs = {PRETRAINING: 1e-4, TRAINING: lr}
        for step in [PRETRAINING, TRAINING]:
            optimizer = optim.SGD(model.parameters(), lr=lrs[step])
